        ...
        <div class="ico-item_fundraising">
          <div class="ico-item_cap">
            ...
            <div class="ico-item_scale">
              ...
              <div :class="['ico-item_scale-inner',
                            {'ico-item_scale-inner--green': ico.status === 'completed'},
                            {'ico-item_scale-inner--red': ico.status === 'canceled'}]"
                            :style="[{width: `${countStylePercent(ico)}%`}]"></div>
            </div>
            <div class="ico-item_cap-current" v-if="ico.status === 'completed'">
              <span :class="['ico-item_cap-current-usd',
                            {'ico-item_cap-current-usd--reached': ico.status === 'completed'},
                            {'ico-item_cap-current-usd--canceled': ico.status === 'canceled'}]" >{{currencyConverter( ico.current_cap, currentCurrency.ticker, false )}} <sup>{{ currentCurrency.ticker }}</sup></span>
              <span class="ico-item_cap-current-percent">{{ countPercent(ico) }}%</span>
            </div>
          </div>
          <!-- /.ico-item_cap -->
          <div class="ico-item_status-wrapper">
            <span v-if="ico.status === 'canceled'" class="ico-item_status ico-item_status--canceled">{{ $t('favourite.ico_canceled') }}</span>
            <span v-if="ico.status === 'completed'" class="ico-item_status ico-item_status--reached">{{ $t('favourite.soft_reached') }} {{ completedICO(ico.starts, ico.completed) }} DAYS</span>
          </div>
          <!-- /.ico-item_price -->
        </div>
...
export default {
  ...
  methods: {
    ...
    completedICO (startDate, completeDate) {
      // let diff = this.$moment(completeDate).unix() - this.$moment(startDate).unix();
      // return this.$moment.unix(diff).format('D')
      // return this.$moment(completeDate).diff(completeDate, 'days')
      let starts = this.$moment(startDate)
      let ends = this.$moment(completeDate)
      let duration = this.$moment.duration(ends.diff(starts))
